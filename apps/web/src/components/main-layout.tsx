...
export const MainLayout = ({
  ...
  // TODO : ensure it's kept in context to keep the state when navigating
  ...
  return (
    <div className="text-white flex flex-col bg-header w-full max-w-[1440px] mx-auto relative">
      ...
      <div className="flex w-full flex-grow overflow-hidden">
        ...
        <div
          className={cn(
            'hidden lg:block fixed top-20 w-6 h-6 z-50',
            isSidebarOpen
        ...
        <div
          className={cn(
            'flex flex-col w-full lg:mt-20 lg:min-h-[calc(100vh - 5rem)] overflow-hidden transition-all ease-in-out bg-white',
...
const SideBar = ({ isSidebarOpen }: { isSidebarOpen: boolean }) => {
  ...
  const [currentTab, setCurrentTab] = useState<'learn' | 'admin' | 'teach'>(
    'learn',
  );
  ...
  return (
    <nav
      className={cn(
        'fixed flex flex-col top-20 px-4 h-[calc(100vh-5rem)] overflow-y-auto max-lg:hidden no-scrollbar transition-all ease-in-out gap-3',
