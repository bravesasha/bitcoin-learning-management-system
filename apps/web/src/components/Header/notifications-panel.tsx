      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(
              'max-lg:hidden group z-50 flex size-13 items-center justify-center gap-2 rounded-2xl text-sm font-semibold outline-hidden transition-all hover:bg-darkOrange-4 lg:gap-2.5 relative',
              className,
            )}
            aria-label="Toggle notifications panel"
          >
            <FaBell className="text-newBlack-1" size={20} />
            <div className="absolute top-4 right-3 rounded-full size-3 bg-darkOrange-2" />
          </button>
        </PopoverTrigger>
        <PopoverContent
          className={cn(
